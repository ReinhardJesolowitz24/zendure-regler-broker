...
- **Netzteil (Empfehlung):** Die ~1,5 W sind der *Bedarf* — im Dauerbetrieb sollte ein Netzteil aber nicht am Limit laufen. Für ein Netzteil, das **beide** versorgt, ist **≥ ~2,5 W** (5 V / ≥ 0,5 A) ein sinnvolles Minimum: Reserve gegen Boot-/Aktivitätsspitzen und thermische Marginale. Jedes gängige 5-V-USB-Netzteil (≥ 1 A) erfüllt das locker; mehr Reserve (wie unser 12-W-Anker) schadet nicht. Wer den Stack nur als **Störungs-Option** statt als Default betreibt, darf kleiner dimensionieren — letztlich Nutzer-Entscheidung. Praktisch: **jeder spec-konforme USB-A-Port liefert 2,5 W** (USB 2.0: 500 mA @ 5 V; USB 3.x: 4,5 W) — beide Geräte (~1,5 W) passen in *einen* Port und lassen sich per **Y-Kabel** gemeinsam versorgen. Ein dedizierter Ladeport (≥ 1 A) gibt zusätzlich Reserve für Einschaltspitzen ggü. einem PC-Port am 500-mA-Limit.
